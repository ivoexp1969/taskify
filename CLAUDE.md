...
(`SharedTasksSection`): обособена секция „Споделени" НАЙ-ОТГОРЕ в `task_screen` (ListView index 0), показва
днешните+просрочените (и завършени днес) групови задачи от ВСИЧКИ групи наведнъж, всяка с етикет на групата;
real-time (комбинира `GroupService.watchMyGroups`→`watchTasks` per група, ръчен combineLatest с коректен
dispose). Завършване=`toggleComplete` (author-gated), отваряне→`GroupTasksScreen`. ★Моделите РАЗДЕЛЕНИ★
(лични Task/Hive vs GroupTask/Firestore). Скрива се, ако няма групи/задачи. **PC (08-07):** `flutter clean`
→ AAB (59.1MB, merged manifest пази USE_FULL_SCREEN_INTENT) + APK (77MB), APK инсталиран+пуснат на Note 9
(SM-N960F) без краш. ★Play release notes лимит 500 знака → съкратени EN(373)+BG(374) в scratchpad★. `analyze`
0 грешки, web+iOS build ✅.**
ПРЕДИШНО:
...
- **★Android настигна iOS → v1.0.57+67 ЖИВА в Play Production 100% (PC, 2026-08-07):** изпълних Android
  ...
  Dart → важи 1:1 за Android без промени. `analyze` **0 грешки**. `flutter clean` → AAB (59.1MB, merged
  manifest пази USE_FULL_SCREEN_INTENT) + APK (77MB). APK инсталиран+пуснат на **Note 9 (SM-N960F)** без
  краш (logcat чист). AAB качен **Production 100%** през `tools/play_upload.py --status completed`
