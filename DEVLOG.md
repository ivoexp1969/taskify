...
Студент ръчни `StudentKeyDate`), БЕЗ нови SchoolTerms/UniversitySemesters. Реализирани 4 секции:
- **Ф7 компактна Профил карта** (`_compactProfile`): Ученик `🎒 Ученик · N клас` + училище; Студент
  `🎓 Студент · ВУЗ` + `курс · специалност · форма` (`_courseLabel` / `_formLabels`). Тап → grade sheet /
  `StudentOnboardingScreen`.
- **Ф2 обратно броене:** преизползвана готовата `StudyCountdownCard` (остава и на таб Задачи — извън обхват).
- **Ф3 днешно разписание — ново `widgets/today_schedule_card.dart`** (`TodayScheduleCard`): часовете за
  деня от текущия срок; маркира „сега" (ярко) / „следваща (след X мин/ч)" / минали (избледнели). Празно →
  „нямаш часове ☕" или ваканция → „Приятен ден! 🌴"; изцяло празно → подкана „Въведи разписанието →".
  Реактивно през `WeeklyScheduleService.revision`.
- **Ф5 днешни задачи — ново `widgets/study_today_tasks_card.dart`** (`StudyTodayTasksCard`): до 4 задачи
  `taskKind∈homework/essay/coursework`, срок днес+3 дни, `ValueListenable` на Hive box; бутон „Виж всички"
  → таб Задачи през нов `ModesScreen.onOpenTasks` (`home_screen` сетва `_currentIndex=0`). Скрита ако няма.
- **Ф6 предстоящи изпити:** Ученик = готовата `ExamHelperCard`; Студент = нова секция „🎯 Предстоящи изпити"
  (`_studentExams`, ключови дати `kind==exam`).
- Махнат мъртъв код от `modes_screen` (`_off`/`_setUp`/`_modeCard`). Всичко 11 ез. inline. `analyze` = **0**.
- **НЕ реализирано (Иво: „остави така"):** авто-филтър „само Обучение" за бутона Ф5; махане на countdown от таб Задачи.
...
- **V1 (6 фази):** онбординг Ученик(клас+училище)/Студент(ВУЗ от `universities_bg.json`, факултет=free-text);
  обратно броене (Ученик→ваканция/изпит от Firestore `school_calendar`; Студент→ръчни ключови дати);
  тип задача Домашно/Есе/Курсова + авто-подзадачи (`task_templates.json`, `planSubtasks`); седмично
  разписание; НВО/ДЗИ помощник (4/7/10/12 клас). `Task.taskKind` HiveField(24). Тестове ✅, analyze 0.
- **UX:** режимите взаимно изключващи се; таб „Уча"→**„📚 Обучение"**, показва само активния режим; избор
  на роля от **Настройки→Училищен режим**; специалност поправена (един контрол); „разпис"→„разписание"+
  триене; ключови дати общи + „Начало на учебната година"; **8 падащи групи** в Настройки; споделени
  задачи **само от автора**; календар — разписанието се редува при >1 час.
- **★НЕЗАВЪРШЕНО (WIP):** картата за броене на началния екран да **редува няколко най-близки СЪБИТИЯ**
  (само `SchoolCountdownCard.showExamLine` param добавен; ротаторът в `study_countdown_card.dart` остава).
- **★НОВИ TODO (заявени 07-23, НЕ имплементирани):**
  1. **Валидация разписание:** да НЕ се допускат учебни часове/събития с **еднакъв час** (припокриващо
     се време) в един и същ ден — предупреди/блокирай в диалога за слот.
  2. **„Учебна програма" в таб Обучение:** нов бутон „Учебна програма", организиран **по учебни срокове
     (Ученик) / семестри (Студент)** — там се въвежда разписанието (разписание per срок/семестър, не общо).
- **ОСТАВА за Иво:** попълни Firestore НВО/ДЗИ дати (след заповед МОН); прегледай `universities_bg.json`.

## 2026-07-21 · Mac — Категория „🎒 Училище" (под-избор на предмети) + iOS 1.0.54 (64) в App Store
Изпълних TODO/handoff-а от 07-20 (по-долу).
- **Училище чип:** предметите `subj_*` вече не задръстват селектора — скрити зад **един чип „🎒 Училище"**,
  който отваря **шийт** за избор на предмет; крайната задача се категоризира по конкретния предмет
  (статистики/филтри работят). UI групиране (без адаптер bump). В `task_screen` (споделен през
  TaskEditorBridge → Задачи+Споделени) + собствения селектор на `calendar_screen`. Нов `_pickSchoolSubject`
  + `_schoolLabel` (11 ез.) и в двата екрана. analyze 0 грешки; тествано на Toto.
- **iOS App Store:** 1.0.53 вече READY_FOR_SALE → нова версия **1.0.54** → build 64 upload → attach → submit →
  **WAITING_FOR_REVIEW**. whatsNew EN+BG (Училищен режим + чипа), без емоджи. **★ЗА PC: Android 1.0.54+64**
  (същият код; version bump + „Какво ново" вече е 64).
- **Следва (нов промпт от Иво):** режими Ученик+**Студент** + обединяване на Споделени в Задачи + нов таб
  **„Уча 🎓"**. Фаза 0 discovery готова; решения на Иво: няма реални Споделени потребители (по-агресивно ОК);
  таб „Уча 🎓"; курсове 1–6. Предстои имплементация.

## ⏳ TODO / HANDOFF → Mac (заявка на Иво, 2026-07-20)
**Категория „Училище" с под-избор на предмети (по-добър UX от плоския списък).**
Вместо всеки предмет да е ОТДЕЛНА категория най-горе (сегашно поведение — задръства селектора
на категории), направи **една категория „Училище"**, която при избор отваря **избраните от
ученика предмети** и той избира от тях.
- **Идея:** в редактора на задача при избор на „Училище" → отваря се лист/шийт с предметите на
  ученика (категориите `subj_*`, които е добавил от Училищен режим) → избира един. Крайната задача
  се категоризира по конкретния предмет (за да работят статистики/филтри), но входът е ЕДНА чиста
  категория „Училище".
- **Реализация (предложение):** моделът `Category` е плосък (id/name/colorValue/isDefault) — БЕЗ
  parent. Най-лесно е **UI групиране**: скрий `subj_*` от плоския ред с категории и ги събери под
  синтетичен елемент „🎒 Училище" (expander/шийт) в селектора на категории (виж `task_screen.dart`
  избор на категория, ~ред 296 `categoryBox.values.map`). Предметите остават реални категории.
  Алтернатива: добави поле `parentId` към `Category` (нов HiveField) — по-чисто, но иска адаптер bump.
- Предметите се добавят от Настройки → България → Училищен режим → „Добави предмети"
  (`_pickSubjects`, grade-aware, id-та `subj_*` + `subj_custom_*`).
- Чист Dart → важи и за Android, и за iOS (който го имплементира, важи за двете). Който пипне UI-я на
  категориите — да тества и трите екрана (Задачи/Календар/Споделени).

## 2026-07-20 · PC — „Училищен режим" (българска учебна година + обратно броене до ваканция), без app bump
Нова функция за ученици; ядрото = **обратно броене до следваща ваканция** („Още N дни до Коледната
ваканция 🎄") — куката, която Todoist/Google Calendar нямат: приложението знае БГ учебната година.
Архитектурата е **1:1 копие на `renewal_offers` remote config**: Firestore колекция `school_calendar`
(документ по учебна година, напр. `bg_2026_2027`) + вграден JSON fallback + 24ч кеш + `revision`
ValueNotifier за реактивност. ★Датите НЕ са хардкоднати в билда★ — Иво ги пълни в един Firestore
документ веднъж годишно по заповед на МОН, без нов билд. При липса на документ → **честно празно
състояние**, НЕ гадаем.
- **Нови файлове:** `lib/models/school_calendar.dart` (чиста, тествана логика — `computeCountdown`,
  `gradeMatches`, `SchoolYear`/`SchoolPeriod`/`SchoolExam`), `lib/services/school_calendar_service.dart`
  (кеш→Firestore→bundled, `enabledNotifier`+`revision`, клас в SharedPreferences),
  `lib/widgets/school_countdown_card.dart` (реактивна карта, 11 езика вкл. руски 3 форми за „ден/дни"),
  `assets/data/school_calendar_bg.json` (★seed 2025/2026, `official:false` + TODO — плейсхолдър дати★),
  `test/school_calendar_test.dart` (**14 теста, всички минават** — ваканция днес, последен учебен ден,
  свършила година, различен край по клас, липса на данни).
- **Ключова тънкост:** краят на годината е РАЗЛИЧЕН по клас (I–III, IV–VI, VII–XI, XII) → потребителят
  ИЗБИРА клас при включване (иначе броенето е грешно). НВО/ДЗИ карти само при съвпадащ клас (`grades`).
- **Интеграция:** карта в Tasks екрана под productivity банера (сама се скрива при изключен режим);
  toggle „Училищен режим" + избор на клас + „Добави училищни предмети" (Фаза 3 — предметите стават
  обикновени категории, преизползва съществуващата система) в BG секцията на Настройки; `main.dart`
  зарежда при старт само ако е вкл.; `firestore.rules` — ново read правило за `school_calendar`.
- **РЕШЕНИЕ:** режимът е **БЕЗПЛАТЕН** (виралната кука движи инсталации; монетизацията е другаде). Лесно
  за Pro-gate (увий toggle-а в `showPaywallIfNeeded`, като именните дни).
- **Чист cross-platform Dart** — БЕЗ native/нови plugins/permissions → iOS = само Mac билд (Firestore
  вече е в Podfile). `flutter analyze` на новите файлове = 0 issues.
- **Тествано на живо (USB, Note 9):** инсталира и стартира без краш; toggle/избор клас/диалог предмети
  се рендерират коректно. `firestore.rules` вече **ДЕПЛОЙНАТИ** в продукция (`taskify-1969`).
- **Follow-up (същия ден, по обратна връзка):** (1) предметите вече са **grade-aware** (`min..max` клас —
  начални класове нямат Физика/Химия/Биология; те са от 7.; История/География от 5.; ново „Човекът и
  природата" 1–6) + **собствен предмет** (свободен текст) в диалога; (2) **инструкции „в началото"** —
  еднократен интро диалог на home за BG (`school_intro_shown`, „Включи от Настройки → България"); (3)
  **„Как се ползва"** tile в секцията (стъпков диалог, `SettingsScreen.showSchoolHowTo`) + инструкция под
  toggle-а при изключен режим. Всичко 11 ез.; analyze 0 нови грешки.
- **ОСТАВА за Иво:** попълни реалните дати в Firestore Console (`school_calendar/bg_2026_2027` по заповед
  на МОН, `official:true`) — има готов `tools/school_calendar_bg_2026_2027.template.json` +
  `tools/school_calendar_upload.py`. Пропусната (опц.): Phase 3 AI „домашно по математика" (server-side).
- **БЪДЕЩО (идея на Иво):** „студентски режим" за университет — семестри/изпитни сесии/дати на изпити
  вместо ваканции; същата remote-config архитектура, отделен документ/тип.

## 2026-07-15 · PC — Именник SEO: значения на имена (де-thin) + og:image/breadcrumb/lastmod (само уеб, без app bump)
Задача „SEO страници за имена дни" — ★оказа се, че системата ВЕЧЕ съществува и е жива★:
`tools/nameday_seo.py` (от 2026-07-11, картичките) генерира **821 страници** от `bg_name_days.json`
(700 по име `/imen-den/ime/{slug}/`, 120 по дата `/imen-den/data/{mm-dd}/`, хъб, sitemap; movable =
православен Великден + offset, динамично per година). Единствената реална дупка = **тънки/еднакви
страници**. Потребителят избра „40 имена с уникален текст" (не всички 700).
- **`MEANINGS` (40 популярни имена)** — авторски преформулиран текст за произхода/значението (2-3 изр.,
  НЕ копиран) → `meaning_block()` инжектира H2 „Какво означава името X?" + абзац + втори FAQ въпрос в JSON-LD.
- **★РИСК 2★ плаващи празници:** изрична бележка „подвижен празник — датата се мени спрямо Великден,
  показана за {year} г." (важи за всичките **46 movable имена**).
- **Бързи SEO подобрения:** `og:image`+Twitter Card (`og-image.png`, живо → споделяне в Messenger/Viber
  с картинка), **BreadcrumbList** JSON-LD (име + дата страници), `<lastmod>` в sitemap. `page()` вече
  приема списък JSON-LD.
- Verify: 821 страници, **всички 821 title-а уникални**, всички JSON-LD блокове парснати валидно;
  deploy `--project-name=taskify` (★сайт=`taskify`, app=`taskify-app`★), верифицирано живо с curl.
- **Хостинг разчистено:** сайтът е на **Cloudflare Pages** (проект `taskify`), Namecheap = само домейн;
  безплатен план, 868 файла от лимит 20 000 → грамаден запас. Commits `91b8b52`, `<seo-quickwins>`.
- **TODO за Иво (дати за евент. проверка, НЕ грешки):** Валентин (10 фев vs 14 фев), Мария (8 септ vs
  15 авг), Илия (2 авг „по стар стил") — datasetът е същият като в production app-а.
...
сянка+контур (четимо на светла И тъмна снимка) + бутон „🔄 Смени фон" + **fallback към градиента**, ако
снимка липсва (нищо не се чупи). CORS-safe (локални снимки, същия домейн → `toBlob` работи).
- **8 реални снимки СВАЛЕНИ, проверени и оптимизирани ТУК** (Claude има мрежов достъп): Unsplash CDN
...
- **UX итерации след тест на Note 9 (същия ден):** (1) диалогът „Очаквайте скоро" 🌸 потвърден на живо;
  (2) бутонът `GiftOfferButton` → `OutlinedButton` с **неоново-зелен ТЕКСТ+кант** (`0xFF39FF14`), не плътен фон
  (плътното зелено бе грозно); (3) при ИМЕН ДЕН опцията е САМО в менюто на конкретния контакт
  (`_showContactActions`) — общият бутон в банера бе безсмислен без получател → МАХНАТ; при РОЖДЕН ДЕН остава
  на картата; (4) ★фикс★ менюто на контакта е `showModalBottomSheet` без `isScrollControlled` → при 7+ реда
  долните (вкл. цветя) се отрязваха под лимита 9/16 → добавен `isScrollControlled:true` + `SingleChildScrollView`;
  (5) ★фикс★ имен-ден банерът с ~30 имена (7 юли) изтласкваше „От твоите контакти"/задачите извън екрана →
  целият долен блок (банери+заглавие+задачи) вече е в ЕДИН `ListView` (банерите като хедър) → скролва.
- **Рожден-ден нотификация:** action „🌸 Цветя/подарък" + payload `gift:birthday`; warm/cold deep-link →
  диалог „Очаквайте скоро" (`setGiftTapCallback` в `main.dart`, cold-start през `gift_pending_route` в home).
- ⚠️ **Repo не билдва от чист клон / на Mac:** `main.dart` (в HEAD) реферира `conversion_service.dart` +
