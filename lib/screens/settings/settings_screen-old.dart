        ...
        children: [
                ...
                if (_isCalendarConnected) ...[
                  ...
                  ListTile(
                    ...
                    onTap: () async {
                      ...
                      // Вземаме default категория
                      const String catIdCalendar = 'sys_calendar_events';
                      const String catIdBirthdays = 'sys_birthdays';
                      const String catIdOoo = 'sys_out_of_office';
                      const String catIdFocus = 'sys_focus_time';
                      const String catIdLocation = 'sys_work_location';
                      
                      Future<String> getOrCreateCat(String catId, String catName, int catColor) async {
                        try {
                          final cat = categoryBox.values.firstWhere((c) => c.id == catId);
                          return cat.id;
                        } catch (_) {
                          final newCat = Category(id: catId, name: catName, colorValue: catColor);
                          await categoryBox.add(newCat);
                          return newCat.id;
                        }
                      }
                      
                      final existingIds = taskBox.values.where((t) => t.googleCalendarEventId != null).map((t) => t.googleCalendarEventId).toSet();
                      
                      // Also track by title+date to avoid duplicates from different sources
                      ...
                      for (final event in events) {
                        if (event['status'] == 'cancelled' || event['recurrence'] != null) {
                          skipped++;
                          continue;
                        }
                        
                        final rawId = event['id'] as String?;
                        // Strip birthday instance suffix (_YYYYMMDD)
                        final eventId = rawId != null && rawId.contains('_') && rawId.split('_').last.length == 8
                            ? rawId.split('_').first
                            : rawId;
                        ...
                        final eventType = event['eventType'] ?? 'default';
                        if (eventType != 'birthday') {
                          print('Non-birthday: ${event['summary']} - Type: $eventType - Date: ${event['start']}');
                        }
                        DateTime? dueDate;
                        
                        try {
                          final startObj = event['start'];
                          if (startObj != null) {
                            if (startObj['dateTime'] != null) {
                              dueDate = DateTime.parse(startObj['dateTime'] as String).toLocal();
                            } else if (startObj['date'] != null) {
                              dueDate = DateTime.parse(startObj['date'] as String).add(const Duration(hours: 12));
                            }
                          }
                        } catch (e) {
                          skipped++;
                        ...
                        if (eventType != 'birthday' && dueDate.isBefore(threshold)) {
                          print('SKIP old event: ${event['summary']} - Date: $dueDate');
                          skipped++;
                          continue;
                        }
                        
                        // Check for duplicate by title+date (ignores time)
                        final title = event['summary'] as String? ?? 'Untitled Event';
                        final dateKey = '${title}_${dueDate.year}-${dueDate.month}-${dueDate.day}';
                        if (existingTitleDates.contains(dateKey)) {
                          print('SKIP duplicate title+date: $title - Date: ${dueDate.year}-${dueDate.month}-${dueDate.day}');
                          skipped++;
                          continue;
                        }
                        existingTitleDates.add(dateKey);
                      ...
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text('Imported: $imported${skipped > 0 ? ', Skipped: $skipped' : ''}'),
                            duration: const Duration(seconds: 3),
                          ),
                        );
                      }
                  ...
                  ListTile(
                    ...
                    title: const Text('Delete All Calendar Tasks'),
                    ...
                    onTap: () async {
                      ...
                      final confirm = await showDialog<bool>(
                        ...
                        builder: (context) => AlertDialog(
                          title: const Text('Delete All Calendar Tasks?'),
                          content: const Text('This will delete all tasks imported from Google Calendar. This action cannot be undone.'),
                          actions: [
                            TextButton(
          ...
          Card(
            child: SwitchListTile(
              ...
              subtitle: const Text('Daily task summary at 8:00 AM'),
